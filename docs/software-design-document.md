...
- Use a local, versioned WCAG 2.2 A/AA knowledge package so normal audits never
  fetch W3C documentation and old reports can identify the dataset revision.
- Add AI-assisted review only after deterministic analysis, with bounded
  criterion-specific tasks and separate provenance from automated findings.
...
- AI certification, AI-adjusted scoring, or asking a model to audit an entire
  website for WCAG conformance.
...
| `packages/engine-*`          | Engine-specific adapters for axe-core, IBM Equal Access, Alfa, QualWeb, and HTML_CodeSniffer.                                                                                                                 |
| `packages/analyzer-keyboard` | Keyboard interaction analyzer.                                                                                                                                                                                |
| `packages/analyzer-ai`       | Ally-owned semantic review task builders, candidate discovery, prompt safety, structured output validation, and provider boundary.                                                                            |
| `packages/wcag`              | Versioned local WCAG 2.2 A/AA criterion data, informative guidance summaries, and Ally-specific coverage metadata.                                                                                            |
| `packages/markup-validator`  | Optional Nu HTML Checker markup diagnostics, stored separately from accessibility findings and score.                                                                                                         |
| `packages/remediation`       | Optional deterministic recommendations for deduplicated findings.                                                                                                                                             |
...
Completed `AuditResult`s may also contain:

- `wcagReview`: report-level WCAG 2.2 A/AA coverage using the local
  `@ally/wcag` dataset revision.
- `aiReview`: optional AI-assisted review tasks/results. Pending tasks are
  generated by the runner from bounded DOM evidence; hosted Workers AI results
  are added by the Worker before R2 persistence. Raw prompts, full-page HTML,
  and hidden model reasoning are not persisted.
